...
- **Skill 系统**: YAML 格式的 skill 模板，支持本地模型加载后注入 ChatGPT 会话
  - 内置 `code_reviewer`、`paper_section_writer`、`kgqa_prompt_audit` 三个 skill
- **Admin API - 云端会话**: `GET /admin/chatgpt/conversations` 和 `GET /admin/chatgpt/conversation/:id` 获取云端 ChatGPT 会话
- **Admin API - Skill 执行**: `POST /admin/chatgpt/skill/execute` 通过 Admin API 执行 skill
- **会话同步**: `chatgpt_sessions` 工具新增 `sync` 操作，以云端为准同步本地会话
- **会话路由**: 支持 `topic` 自动匹配已有会话、`conversation_url` 精确继续

### 🔒 Security
- MCP server 不再硬编码 API 密钥，改为环境变量必填
