> Historical documentation, superseded by the repository README and DEPLOYMENT.md on 2026-09-10. Model API and polling examples below are retired.
...
## Current ChatGPT UI acceptance smoke

After deployment and after the bridge browser is authenticated, run:

```bash
npm run smoke:chatgpt
```

The smoke checks session status, lists one conversation, dispatches a throwaway chat, waits for the exact marker reply, and deletes the throwaway conversation. It never prints cookies, API keys, or access tokens.
