    ...
    async function readBrowserSession(page) {
        return page.evaluate(async () => {
            try {
                ...
                const cookieAuthenticated = meResponse.ok && !!(me?.id || me?.email || me?.name);
                return {
                    ok: !!accessToken || cookieAuthenticated,
                    accessToken,
                    authMode: accessToken ? 'bearer-and-cookies' : (cookieAuthenticated ? 'cookies' : null),
                    sessionHttpStatus,
                    backendMeHttpStatus: meResponse.status,
                    sessionKeys: Object.keys(session || {}),
                    userPresent: cookieAuthenticated || !!session?.user,
                };
            } catch (error) {
    ...
    async function inspect({ allowRestore = true, alert = true, persist = true } = {}) {
        ...
        if (session.ok) {
            ...
            return {
                loggedIn: true,
                state: 'logged-in',
                authMode: session.authMode || 'cookies',
                restoredCookies,
                persisted,
                sessionKeys: session.sessionKeys || [],
                backendMeHttpStatus: session.backendMeHttpStatus || null,
