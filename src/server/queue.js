...
export function createQueueManager(queueConfig, callbacks) {
    ...
    // Browser control lock used by exact dispatch/steer. It prevents a newly queued
    // normal generation from navigating the single browser halfway through an atomic
    // Stop -> wait -> send control operation. An already-processing target response is
    // intentionally allowed to continue so the control operation can interrupt it.
    let browserControlLocked = false;
    ...
    async function processTask(task) {
        ...
        try {
            ...
            if (isStreaming) {
                const chunk = buildChatCompletionChunk(finalContent, modelName, 'stop', reasoningContent, resultConvUrl);
                sendSse(res, chunk);
                sendSseDone(res);
                logger.info('服务器', '流式响应已结束', { id });
            } else {
                const response = buildChatCompletion(finalContent, modelName, reasoningContent, resultConvUrl);
    ...
    async function processQueue() {
        // An atomic browser-control operation owns the single page. New normal tasks
        // remain queued until the lock releases; the currently processing task, if any,
        // is not cancelled because it may be the response being steered.
        if (browserControlLocked) {
            return;
        }
    ...
    function getStatus() {
        return {
            ...
            browserControlLocked,
            browserControlReason
        };
    }
    ...
    /**
     * Try to acquire exclusive control of the single browser page for an atomic
     * admin operation. Returns a release function, or null if another control
     * operation already owns the page.
     */
    ...
    async function initializePool() {
        poolContext = await initBrowser(config);
        // 初始化完成后，触发首次监控跳转
        if (navigateToMonitor && !browserControlLocked) {
            navigateToMonitor().catch(() => { });
        }
        return poolContext;
    }
